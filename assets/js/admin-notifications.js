 (function ($) {
    ...
    $(document).ready(function () {
        ...
        if (config.pwa_enabled === 'on' && 'Notification' in window) {
        ...
        if (config.pwa_enabled === 'on' && config.manifest_url) {
            addManifestLink(config.manifest_url);
        }
    ...
    function showNotification(alert, message) {
        const payload = {
            title: alert.label || alert.type || 'Notification',
            body: message,
            icon: config.pwa_icon || '',
            badge: config.pwa_badge || config.pwa_icon || '',
            tag: alert.id || 'mulopimfwc-notification',
            requireInteraction: alert.severity === 'critical',
            data: {
                url: alert.data?.url || '/wp-admin/admin.php?page=multi-location-product-and-inventory-management',
                type: alert.type
            },
