...
class Plugincylwp_Product_Location_Table extends WP_List_Table
{
    ...
    private function get_actions_display($item)
    {
        $locations = $item['location_terms'];
        if (empty($locations)) {
            return '<a href="#" class="button button-small add-location" data-product-id="' . esc_attr($item['id']) . '">' .
                __('Add to Location', 'location-wise-product') . '</a>';
        }
        ...
        if (isset($_REQUEST['filter-by-location']) && !empty($_REQUEST['filter-by-location'])) {
            $args['tax_query'] = [
                [
                    ...
                    'field'    => 'slug',
                    'terms'    => sanitize_text_field($_REQUEST['filter-by-location']),
                ],
            ];
        }
