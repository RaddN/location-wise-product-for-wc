...
- REST API namespace: `mulopimfwc/v1`.
- Inventory bulk sync, single update, export, locations, products, webhook, and service-worker routes.
...
- Service-worker support through `/mulopimfwc-sw.js` and the REST fallback route.
- Location resolver supports aliases, selected/current location contexts, request values, cookies, and native location archives.
...
6. **Test Frontend:** Visit shop, single product, cart, checkout, and order pages, select a location, and verify product, price, stock, cart, and order behavior.
...
🔹 POS/WMS/ERP sync is available through REST API and webhook endpoints, not through named vendor-specific native connectors.
...
Depending on settings and customer actions, the plugin may store or process:

🔹 Selected customer location in cookies and WooCommerce cart/order metadata.
🔹 Location-specific stock, price, currency, shipping, payment, tax, pickup, review, SEO, and business-hour metadata.
🔹 Location manager user assignments and permissions.
🔹 Customer location insight events, product views, purchases, selected locations, and recommendation data when tracking/recommendation features are enabled.
🔹 Customer address/location entries when customer location-address features are used.
🔹 API keys, webhook secrets, import/export job data, logs, and artifacts.
🔹 Webhook log entries when webhook logging is enabled.
🔹 Custom Text Management values, notification settings, service-worker notification preferences, and notification test data.
🔹 Deactivation feedback reason when the feedback form is submitted.
🔹 In the EDD build, data-sharing/analytics can send site and plugin metadata to Plugincy endpoints when enabled by plugin settings.

To learn more about Plugincy data policies, please see our [Privacy Policy](https://plugincy.com/usage-tracking/).
...
* Added: shortcode attribute reference, admin tab map, Text Management, notification, service-worker, location archive, and shipping-method documentation.
* Documented: CSV/ZIP import/export support and native XLS/XLSX import limitation.
* Documented: OpenStreetMap/Nominatim/Leaflet usage and Google Maps/MaxMind limitations.
...
* Changed: dashboard “Total Investment” display and related Pro-gated UI
...
* Added: Location-specific pricing and location currency behavior
...
* Added: WooCommerce Blocks support paths
* Added: REST API filtering and inventory sync capabilities
...
* Added: Location matching and auto-detect customer location flows
* Added: Team management with location manager roles
* Added: Advanced analytics with visual charts
* Added: Export reports and analytics data
