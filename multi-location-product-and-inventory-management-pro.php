...
if (!defined('MULOPIMFWC_VERSION')) {
    define("MULOPIMFWC_VERSION", "1.1.8");
}
...
if (!function_exists('mulopimfwc_get_currency_symbol_from_map')) {
     ...
     * @return string
     */
    function mulopimfwc_get_currency_symbol_from_map($currency_code)
    {
        $currency_code = strtoupper(trim((string) $currency_code));
        if ($currency_code === '' || !function_exists('get_woocommerce_currency_symbols')) {
            return '';
        }

        $symbols = (array) get_woocommerce_currency_symbols();
        $symbol = isset($symbols[$currency_code]) ? (string) $symbols[$currency_code] : '';
        if ($symbol === '') {
            return '';
        }
...
if (!function_exists('mulopimfwc_get_unfiltered_currency_symbol')) {
    /**
     * Resolve a display currency symbol by code while bypassing active currency-switcher symbol filters.
     *
     * @param string $currency_code Currency code.
     * @return string
     */
    function mulopimfwc_get_unfiltered_currency_symbol($currency_code)
    {
        $currency_code = strtoupper(trim((string) $currency_code));
        if ($currency_code === '') {
            return '';
        ...
        $symbol = mulopimfwc_get_currency_symbol_from_map($currency_code);

        return $symbol !== '' ? $symbol : $currency_code;
    }
}

if (!function_exists('mulopimfwc_filter_woocommerce_currency_symbol_by_code')) {
    /**
     * Keep WooCommerce price output aligned with the currency code even when another switcher
     * returns its active symbol for every requested currency.
     *
     * @param string $currency_symbol Filtered symbol.
     * @param string $currency        Currency code requested by WooCommerce.
     * @return string
     */
    function mulopimfwc_filter_woocommerce_currency_symbol_by_code($currency_symbol, $currency = '')
    {
        $mapped_symbol = mulopimfwc_get_currency_symbol_from_map($currency);

        return $mapped_symbol !== '' ? $mapped_symbol : $currency_symbol;
    }

    add_filter('woocommerce_currency_symbol', 'mulopimfwc_filter_woocommerce_currency_symbol_by_code', PHP_INT_MAX, 2);
}
...
if (!function_exists('mulopimfwc_is_location_pickup_enabled')) {
    /**
     * Check whether location-based pickup is enabled, respecting manual mode.
     *
     * @param array|null $options Optional options array.
     * @return bool
     */
    function mulopimfwc_is_location_pickup_enabled($options = null): bool
    {
        if (!is_array($options)) {
            global $mulopimfwc_options;
            $options = is_array($mulopimfwc_options ?? null)
...
if (!function_exists('mulopimfwc_get_location_url_prefix')) {
    /**
     * Return the sanitized location URL prefix/query key.
     *
     * @param array|null $options Optional plugin options.
     * @return string
     */
    function mulopimfwc_get_location_url_prefix($options = null): string
    {
        if (!is_array($options)) {
            global $mulopimfwc_options;
            $options = is_array($mulopimfwc_options ?? null)
        ...
        $prefix = isset($options['location_url_prefix']) ? sanitize_title((string) $options['location_url_prefix']) : 'store-location';
...
if (!function_exists('mulopimfwc_get_location_url_query_var')) {
    /**
     * Return the active query-string key for location URLs, if query mode is enabled.
     *
     * @param array|null $options Optional plugin options.
     * @return string
     */
    function mulopimfwc_get_location_url_query_var($options = null): string
    {
        if (!is_array($options)) {
            global $mulopimfwc_options;
            $options = is_array($mulopimfwc_options ?? null)
        ...
        $enabled = isset($options['enable_location_urls']) && $options['enable_location_urls'] === 'on';
        $format = isset($options['url_location_format']) ? sanitize_key((string) $options['url_location_format']) : 'query_param';

        if (!$enabled || $format !== 'query_param') {
            return '';
        }

        return mulopimfwc_get_location_url_prefix($options);
    }
}
...
if (!mulopimfwc_is_woocommerce_active()) {
    add_action('admin_notices', function () {
        echo '<div class="error"><p>' . esc_html__('Location Wise Products requires WooCommerce to be installed and active.', 'multi-location-product-and-inventory-management-pro') . '</p></div>';
    });
    return;
}
        ...
        private function get_runtime_location_slug_for_currency(): string
        {
            if (is_admin() && wp_doing_ajax()) {
                ...
                if ($ajax_action === 'mulopimfwc_dashboard_live_data') {
                    ...
                    if (!$has_explicit_dashboard_location && function_exists('mulopimfwc_get_location_url_query_var')) {
                        $location_url_query_var = mulopimfwc_get_location_url_query_var();
                        $has_explicit_dashboard_location = $location_url_query_var !== '' && isset($_REQUEST[$location_url_query_var]);
                    }
            ...
            if (function_exists('mulopimfwc_get_location_url_query_var')) {
                $location_url_query_var = mulopimfwc_get_location_url_query_var();
                if (
                    $location_url_query_var !== '' &&
                    !isset($candidates['request_location_url_query_var']) &&
                    isset($_REQUEST[$location_url_query_var])
                ) {
                    $candidates['request_location_url_query_var'] = (string) wp_unslash($_REQUEST[$location_url_query_var]);
                }
            }
        ...
        public function validate_location_selection_before_add_to_cart($passed, $product_id, $quantity, $variation_id = 0, $variations = [])
        {
            ...
            if ($selected_location === '' || $selected_location === 'all-products') {
                if (function_exists('wc_add_notice')) {
                    wc_add_notice(__('Please select a store location before adding this product to your cart.', 'multi-location-product-and-inventory-management-pro'), 'error');
                }
                return false;
            }
...
?>
            <div class="mulopimfwc-order-transfer-costs" style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-left: 4px solid #3b82f6; border-radius: 4px;">
                <h4 style="margin-top: 0; color: #1e40af;">
                    ...
                    <?php esc_html_e('Inter-Location Transfer Costs', 'multi-location-product-and-inventory-management-pro'); ?>
                </h4>
                ...
                <table class="widefat" style="margin-top: 10px; border: 1px solid #e2e8f0;">
                    <thead>
                        <tr>
                            <th style="padding: 8px; background: #f1f5f9; border-bottom: 1px solid #e2e8f0;"><?php esc_html_e('From Location', 'multi-location-product-and-inventory-management-pro'); ?></th>
                            <th style="padding: 8px; background: #f1f5f9; border-bottom: 1px solid #e2e8f0;"><?php esc_html_e('To Location', 'multi-location-product-and-inventory-management-pro'); ?></th>
                            <th style="padding: 8px; background: #f1f5f9; border-bottom: 1px solid #e2e8f0; text-align: right;"><?php esc_html_e('Cost', 'multi-location-product-and-inventory-management-pro'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
        ...
        public function add_inter_location_transfer_fees()
        {
            ...
            if ($total_transfer_cost > 0) {
                $fee_label = __('Inter-location Transfer', 'multi-location-product-and-inventory-management-pro');
        ...
        ?>
            <div class="mulopimfwc-transfer-cost-notice woocommerce-info">
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <div>
                        <strong><?php esc_html_e('Transfer Costs Applied', 'multi-location-product-and-inventory-management-pro'); ?></strong>
        ...
        public function display_location_in_cart($item_data, $cart_item)
        {
            ...
            if ($location_name !== '' && ($group_cart !== 'on' || is_cart() || $is_store_api_request)) {
                $item_data[] = array(
                    'key'     => __('Location', 'multi-location-product-and-inventory-management-pro'),
        ...
        public function display_location_in_order_items($item_id, $item, $product)
        {
            ...
            if (empty($available_locations) || is_wp_error($available_locations)) {
                ...
                if (!empty($current_location)) {
                    ...
                    if ($location_term && !is_wp_error($location_term)) {
                        ...
                        echo '<strong>' . esc_html__('Location:', 'multi-location-product-and-inventory-management-pro') . '</strong> ';
            ...
            if ($show_selector) {
                        ...
                        if ($location_stock !== '') {
                            ...
                            if (!empty($stock_data['show']) && $stock_data['label'] !== '') {
                                ...
                                if ($stock_qty < $current_quantity && $location_backorders === 'off') {
                                    $stock_display .= ' - ' . __('Insufficient', 'multi-location-product-and-inventory-management-pro');
                                }
                                $stock_display .= ')';
                            }
                        } else {
                            $stock_display = ' (' . __('No stock set', 'multi-location-product-and-inventory-management-pro') . ')';
                        }
                    }
                ...
                if (!empty($current_location)) {
                    ...
                    echo '<span style="color: #999;">' . esc_html__('Not set', 'multi-location-product-and-inventory-management-pro') . '</span>';
                }

                if ($is_currency_locked) {
                    echo '<span style="display:block;margin-top:4px;color:#666;font-size:11px;">' . esc_html__('Locked while location-wise currency is enabled.', 'multi-location-product-and-inventory-management-pro') . '</span>';
                }
            }
        ...
        public function hide_location_order_itemmeta($hidden_meta)
        {
            ...
            $hidden_meta[] = __('Store Location', 'multi-location-product-and-inventory-management-pro');
        ...
        public function update_order_item_location()
        {
            ...
            if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'mulopimfwc_update_order_item_location')) {
                wp_send_json_error(['message' => __('Security check failed', 'multi-location-product-and-inventory-management-pro')]);
            }

            // Check user permissions
            if (!current_user_can('edit_shop_orders')) {
                wp_send_json_error(['message' => __('You do not have permission to edit orders', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (
                ...
                wp_send_json_error(['message' => __('Location updates are locked when location-wise currency is enabled.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (!$item_id || empty($new_location_slug)) {
                wp_send_json_error(['message' => __('Invalid item ID or location', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (!$order) {
                wp_send_json_error(['message' => __('Order not found', 'multi-location-product-and-inventory-management-pro')]);
            }

            $item = $order->get_item($item_id);
            if (!$item) {
                wp_send_json_error(['message' => __('Order item not found', 'multi-location-product-and-inventory-management-pro')]);
            }

            // Check if order is editable
            if (!$order->is_editable()) {
                wp_send_json_error(['message' => __('This order is no longer editable', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if ($old_location_slug === $new_location_slug) {
                wp_send_json_success(['message' => __('Location unchanged', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (!$new_location_term || is_wp_error($new_location_term)) {
                wp_send_json_error(['message' => __('Location not found', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            wp_send_json_success([
                'message' => __('Location and price updated successfully', 'multi-location-product-and-inventory-management-pro'),
            ...
            foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                ...
                if (!isset($items_by_location[$location])) {
                    $items_by_location[$location] = array(
                        'location_name' => $location_name !== ''
                            ? $location_name
                            : __('Global', 'multi-location-product-and-inventory-management-pro'),
        ...
        public function display_cart_location_summary()
        {
            ...
            if (count($items_by_location) > 1) {
                ...
                echo '<strong>' . esc_html__('Your cart contains items from multiple locations:', 'multi-location-product-and-inventory-management-pro') . '</strong><br>';
        ...
        ?>
            <div class="mulopimfwc-cart-location-selector" data-cart-item-key="<?php echo esc_attr($cart_item_key); ?>" data-product-id="<?php echo esc_attr($product_id); ?>" data-variation-id="<?php echo esc_attr($variation_id); ?>">
                <label for="cart-location-<?php echo esc_attr($cart_item_key); ?>" style="font-size: 0.9em; font-weight: 600; display: block; margin-top: 8px; margin-bottom: 4px;">
                    <?php esc_html_e('Change Location:', 'multi-location-product-and-inventory-management-pro'); ?>
                </label>
        ...
        public function update_cart_item_location()
        {
            ...
            if (empty($cart_item_key) || empty($new_location_slug)) {
                wp_send_json_error(['message' => __('Invalid parameters.', 'multi-location-product-and-inventory-management-pro')]);
            }

            if (!function_exists('WC') || !WC()->cart) {
                wp_send_json_error(['message' => __('Cart not available.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            $options = is_array($mulopimfwc_options ?? null)
                ...
                : get_option('mulopimfwc_display_options', []);
            if (!mulopimfwc_is_location_change_in_cart_enabled($options)) {
                wp_send_json_error(['message' => __('Location change in cart is disabled.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (!$cart_item) {
                wp_send_json_error(['message' => __('Cart item not found.', 'multi-location-product-and-inventory-management-pro')]);
            }

            // Get location term
            $location_term = get_term_by('slug', $new_location_slug, 'mulopimfwc_store_location');
            if (!$location_term || is_wp_error($location_term)) {
                wp_send_json_error(['message' => __('Invalid location.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            wp_send_json_success([
                'message' => __('Location updated successfully.', 'multi-location-product-and-inventory-management-pro'),
                'location_name' => $location_term->name,
        ...
        public function get_cart_item_locations()
        {
            ...
            // Check if product is available in multiple locations
            if (!$this->is_product_available_in_multiple_locations($product_id, $variation_id)) {
                wp_send_json_error(['message' => __('Product is not available in multiple locations.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            $options = is_array($mulopimfwc_options ?? null)
                ...
                if ($enable_all_locations === 'on') {
                    ...
                    wp_send_json_error(['message' => __('Product has no locations assigned.', 'multi-location-product-and-inventory-management-pro')]);
                }
            }
    ...
    <?php
        ...
        public function cymulopimfwc_get_available_locations()
        {
            ...
            if (!$this->current_user_can_manage_products()) {
                wp_send_json_error(['message' => __('You do not have permission to perform this action.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if (!$product_id) {
                wp_send_json_error(['message' => __('Invalid product ID.', 'multi-location-product-and-inventory-management-pro')]);
            }
        ...
        public function cymulopimfwc_save_product_locations()
        {
            ...
            wp_send_json_success([
                'message' => __('Product locations saved successfully.', 'multi-location-product-and-inventory-management-pro'),
            ]);
        }
        ...
        public function cymulopimfwc_update_product_location_status()
        {
            ...
            if (is_array($allowed_location_ids) && !in_array($location_id, $allowed_location_ids, true)) {
                wp_send_json_error(['message' => __('You do not have permission to modify this location.', 'multi-location-product-and-inventory-management-pro')]);
            }
            ...
            if ($action === 'activate') {
                ...
                $message = __('Location activated successfully.', 'multi-location-product-and-inventory-management-pro');
            } else {
                ...
                $message = __('Location removed from product successfully.', 'multi-location-product-and-inventory-management-pro');
            }
        ...
        public function cymulopimfwc_get_product_quick_edit_data()
        {
            ...
            $product = wc_get_product($product_id);
            if (!$product) {
                wp_send_json_error(['message' => __('Product not found.', 'multi-location-product-and-inventory-management-pro')]);
            }

            if (!current_user_can('edit_post', $product_id)) {
                wp_send_json_error(['message' => __('You do not have permission to edit this product.', 'multi-location-product-and-inventory-management-pro')]);
            }
        ...
        public function cymulopimfwc_save_product_quick_edit_data()
        {
            ...
            if (isset($_POST['variations'])) {
                if (!is_array($_POST['variations'])) {
                    wp_send_json_error(['message' => __('Invalid variation payload.', 'multi-location-product-and-inventory-management-pro')]);
                }

                if (!empty($_POST['variations'])) {
                    if ($product_type !== 'variable') {
                        wp_send_json_error(['message' => __('Variations can only be saved for variable products.', 'multi-location-product-and-inventory-management-pro')]);
                    }
                    ...
                    foreach ($_POST['variations'] as $variation_data) {
                        ...
                        if ($variation_id <= 0 || !isset($allowed_variation_lookup[$variation_id])) {
                            wp_send_json_error(['message' => __('One or more submitted variations do not belong to this product.', 'multi-location-product-and-inventory-management-pro')]);
                        }
            ...
            wp_send_json_success(['message' => __('Product data saved successfully.', 'multi-location-product-and-inventory-management-pro')]);
        }
        ...
        public function cymulopimfwc_remove_product_location()
        {
            ...
            wp_send_json_success(['message' => __('Location removed successfully.', 'multi-location-product-and-inventory-management-pro')]);
        }
        ...
        public function cymulopimfwc_enqueue_admin_scripts($hook)
        {
            ...
            $default_currency_symbol = function_exists('mulopimfwc_get_unfiltered_currency_symbol')
                ? (string) mulopimfwc_get_unfiltered_currency_symbol($default_currency_code)
                : '';
            if ($default_currency_symbol === '') {
                $default_currency_symbol = $default_currency_code;
            }

            $text_domain = 'multi-location-product-and-inventory-management-pro';
        ...
        public function save_location_to_order_meta($order_id, $data = null)
        {
            ...
            if ($assignment_method === 'manual') {
                if (mulopimfwc_is_manual_optional_location_selection_enabled($options)) {
                    ...
                    if ($selected_location !== 'all-products' && mulopimfwc_validate_location_slug($selected_location)) {
                        if (!$this->is_location_slug_eligible_for_order($order, $selected_location, $options, is_array($data) ? $data : null)) {
                            $selected_location = '';
                        }
                    } else {
                        $selected_location = '';
                ...
                if ($existing_location === '') {
                    ...
                    if ($current_status !== 'on-hold' && !in_array($current_status, ['cancelled', 'refunded', 'failed'], true)) {
                        $order->update_status(
                            'on-hold',
                            __('Awaiting manual location assignment.', 'multi-location-product-and-inventory-management-pro')
                        );
                    }
                }
            ...
            if ($assignment_method === 'inventory_based') {
                ...
                if (mulopimfwc_is_manual_optional_location_selection_enabled($options)) {
                    ...
                    if ($selected_location === 'all-products' || !mulopimfwc_validate_location_slug($selected_location)) {
                        $selected_location = '';
                    } elseif (!$this->is_location_slug_eligible_for_order($order, $selected_location, $options, is_array($data) ? $data : null)) {
                        $selected_location = '';
                    }
                }
        ...
        private function assign_location_by_inventory($order): string
        {
            ...
            $location_map = $this->filter_location_map_by_order_shipping_zones($location_map, $order);
            if (empty($location_map)) {
                return '';
            }
         ...
         * @return array
         */
        private function get_order_eligible_location_map(WC_Order $order, array $locations, ?array $options = null, ?array $checkout_data = null): array
        {
            if (!is_array($options)) {
                global $mulopimfwc_options;
                $options = is_array($mulopimfwc_options ?? null)
         ...
         * @return bool
         */
        private function is_location_slug_eligible_for_order(WC_Order $order, string $location_slug, ?array $options = null, ?array $checkout_data = null): bool
        {
            ...
            if (!$this->is_location_term_allowed_for_order_shipping_zone($location_id, $order, $checkout_data)) {
                return false;
            }
        ...
        /**
         * Filter candidate locations to those allowed by the order destination's matching shipping zones.
         *
         * Locations without assigned shipping zones remain unrestricted for backwards compatibility.
         *
         * @param array $location_map Candidate location data keyed by slug.
         * @param WC_Order $order Order object.
         * @param array|null $checkout_data Optional checkout data.
         * @return array
         */
        private function filter_location_map_by_order_shipping_zones(array $location_map, WC_Order $order, ?array $checkout_data = null): array
        {
            if (empty($location_map)) {
                return $location_map;
            }

            $destination = $this->get_order_shipping_zone_destination($order, $checkout_data);
            if (empty($destination['country'])) {
                return $location_map;
            }

            $matching_zone_ids = $this->get_matching_shipping_zone_ids_for_destination($destination);
            if (empty($matching_zone_ids)) {
                return $location_map;
            }

            $matching_zone_lookup = array_fill_keys(array_map('intval', $matching_zone_ids), true);
            $has_restricted_locations = false;
            $filtered_location_map = [];
            ...
            foreach ($location_map as $location_slug => $location_data) {
                ...
                if (empty($allowed_zone_ids)) {
                    $filtered_location_map[$location_slug] = $location_data;
                    continue;
                }

                $has_restricted_locations = true;
                foreach ($allowed_zone_ids as $zone_id) {
                    if (isset($matching_zone_lookup[(int) $zone_id])) {
                        $filtered_location_map[$location_slug] = $location_data;
                        break;
                    }
                }
            ...
            return $has_restricted_locations ? $filtered_location_map : $location_map;
        }

        /**
         * Check whether a location's shipping zone restrictions allow an order destination.
         *
         ...
         * @return bool
         */
        private function is_location_term_allowed_for_order_shipping_zone(int $location_id, WC_Order $order, ?array $checkout_data = null): bool
        {
            $allowed_zone_ids = $this->get_location_shipping_zone_ids($location_id);
            if (empty($allowed_zone_ids)) {
                return true;
            ...
            return !empty(array_intersect($allowed_zone_ids, $matching_zone_ids));
        }

        /**
         * Get normalized shipping zone IDs assigned to a location term.
         *
         * @param int $location_id Location term ID.
         * @return int[]
         */
        private function get_location_shipping_zone_ids(int $location_id): array
        {
            $zone_ids = get_term_meta($location_id, 'shipping_zones', true);
            if (!is_array($zone_ids)) {
                $zone_ids = $zone_ids === '' ? [] : [$zone_ids];
            }

            $zone_ids = array_map('absint', $zone_ids);
            return array_values(array_unique(array_filter($zone_ids, static function ($zone_id) {
                return $zone_id >= 0;
            })));
        }

        /**
         * Build the WooCommerce shipping-zone destination for an order.
         *
         * @param WC_Order $order Order object.
         * @param array|null $checkout_data Optional checkout data.
         * @return array
         */
        private function get_order_shipping_zone_destination(WC_Order $order, ?array $checkout_data = null): array
        {
            $address_sets = [
                [
                ...
                [
                    'country' => is_array($checkout_data) ? ($checkout_data['shipping_country'] ?? '') : '',
                    'state' => is_array($checkout_data) ? ($checkout_data['shipping_state'] ?? '') : '',
                    'postcode' => is_array($checkout_data) ? ($checkout_data['shipping_postcode'] ?? '') : '',
                    'city' => is_array($checkout_data) ? ($checkout_data['shipping_city'] ?? '') : '',
                ],
                [
                ...
                [
                    'country' => is_array($checkout_data) ? ($checkout_data['billing_country'] ?? '') : '',
                    'state' => is_array($checkout_data) ? ($checkout_data['billing_state'] ?? '') : '',
                    'postcode' => is_array($checkout_data) ? ($checkout_data['billing_postcode'] ?? '') : '',
                    'city' => is_array($checkout_data) ? ($checkout_data['billing_city'] ?? '') : '',
                ],
            ];

            foreach ($address_sets as $address) {
                $country_code = $this->normalize_country_code_for_shipping_zone($address['country']);
                if ($country_code === '') {
                    continue;
                }

                $postcode = is_scalar($address['postcode']) ? trim(sanitize_text_field((string) $address['postcode'])) : '';
                if ($postcode !== '' && function_exists('wc_normalize_postcode')) {
                    $postcode = wc_normalize_postcode($postcode);
                }

                return [
                    'country' => $country_code,
                    'state' => $this->normalize_state_code_for_shipping_zone($country_code, $address['state']),
                    'postcode' => $postcode,
                    'city' => is_scalar($address['city']) ? trim(sanitize_text_field((string) $address['city'])) : '',
                ];
            }
        ...
        /**
         * Resolve all WooCommerce shipping zones matching a destination.
         *
         * WooCommerce normally uses the first matching zone by zone order. Assignment needs
         * every matching zone so locations can be connected to overlapping zones safely.
         *
         * @param array $destination Destination data.
         * @return int[]
         */
        private function get_matching_shipping_zone_ids_for_destination(array $destination): array
        {
            ...
            $continent = strtoupper((string) WC()->countries->get_continent_code_for_country($country));
            ...
            if ($postcode !== '' && function_exists('wc_postcode_location_matcher')) {
            ...
            $cache[$cache_key] = $zone_ids;
            return $zone_ids;
        }

        /**
         * Normalize a country value into a WooCommerce shipping-zone country code.
         *
         * @param mixed $country Country code or name.
         * @return string
         */
        private function normalize_country_code_for_shipping_zone($country): string
        {
            ...
            return strlen($country) === 2 ? $country : '';
        }

        /**
         * Normalize a state value into a WooCommerce shipping-zone state code.
         *
         * @param string $country_code Two-letter country code.
         * @param mixed $state State code or name.
         * @return string
         */
        private function normalize_state_code_for_shipping_zone(string $country_code, $state): string
        {
            if (!is_scalar($state)) {
                return '';
            }
            ...
            foreach ($states as $state_code => $state_name) {
                ...
                if (strcasecmp(trim($state_name), $state) === 0 || strcasecmp((string) $state_code, $state) === 0) {
                    return strtoupper((string) $state_code);
                }
            }
        ...
        /**
         * Check whether a location lacks enough stock for an order item.
         *
         ...
         * @return bool
         */
        private function is_location_stock_insufficient_for_order_item($item, int $product_id, int $variation_id, int $location_id, ?array $options = null): bool
        {
            if (!is_array($options)) {
                global $mulopimfwc_options;
                $options = is_array($mulopimfwc_options ?? null)
            ...
            if (!isset($options['enable_location_stock']) || $options['enable_location_stock'] !== 'on') {
                return false;
            }

            $quantity = is_object($item) && is_callable([$item, 'get_quantity'])
            ...
            $ids_to_check = array_values(array_filter([absint($variation_id), absint($product_id)]));
            foreach ($ids_to_check as $id) {
                $location_stock = get_post_meta($id, '_location_stock_' . $location_id, true);
                if ($location_stock === '') {
                    continue;
                }

                $location_backorders = function_exists('mulopimfwc_get_effective_location_backorders')
                    ? mulopimfwc_get_effective_location_backorders($id, $location_id)
                    : get_post_meta($id, '_location_backorders_' . $location_id, true);

                if ($this->is_backorder_value_allowed($location_backorders)) {
                    return false;
                }

                return (float) $location_stock < $quantity;
            }

            foreach ($ids_to_check as $id) {
                $product = wc_get_product($id);
                if (!$product || !is_callable([$product, 'managing_stock']) || !$product->managing_stock()) {
                    continue;
                }

                if (is_callable([$product, 'get_backorders']) && $this->is_backorder_value_allowed($product->get_backorders())) {
                    return false;
                }

                $stock_quantity = is_callable([$product, 'get_stock_quantity']) ? $product->get_stock_quantity() : null;
                return $stock_quantity !== null && (float) $stock_quantity < $quantity;
            }
        ...
        /**
         * Normalize WooCommerce and plugin backorder values.
         *
         * @param mixed $backorders Backorder value.
         * @return bool
         */
        private function is_backorder_value_allowed($backorders): bool
        {
            if (function_exists('mulopimfwc_is_backorder_allowed')) {
                return mulopimfwc_is_backorder_allowed($backorders);
            }

            return in_array(strtolower(trim((string) $backorders)), ['on', 'yes', 'notify'], true);
        }
        ...
        private function assign_location_by_proximity($order, ?array $checkout_data = null): string
        {
            ...
            $location_map = $this->get_order_eligible_location_map($order, $locations, $options, $checkout_data);
            if (empty($location_map)) {
                return '';
            }
        ...
        private function geocode_address(string $address, string $country = ''): array
        {
            ...
            $user_agent = sprintf('MulopimFWC/%s (%s)', defined('MULOPIMFWC_VERSION') ? MULOPIMFWC_VERSION : '1.1.8', home_url('/'));
        ...
        public function add_store_location_filter()
        {
            ...
            if ($is_manual_mode) {
                ...
                echo esc_html__('Unassigned', 'multi-location-product-and-inventory-management-pro');
        ...
        public function add_settings_link($links)
        {
            ...
            $old_links = $links;
            $links = [];
            $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=multi-location-product-and-inventory-management-settings')) . '">' . esc_html__('Settings', 'multi-location-product-and-inventory-management-pro') . '</a>';
            $create_location = '<a href="' . esc_url(admin_url('edit-tags.php?taxonomy=mulopimfwc_store_location&post_type=product&orderby=display_order&order=asc')) . '">' . esc_html__('Manage Locations', 'multi-location-product-and-inventory-management-pro') . '</a>';
            $support_link = '<a href="' . esc_url("https://www.plugincy.com/support/") . '">' . esc_html__('Support', 'multi-location-product-and-inventory-management-pro') . '</a>';
            $documentation_link = '<a href="' . esc_url("https://plugincy.com/documentations/multi-location-product-inventory-management-for-woocommerce/") . '">' . esc_html__('Documentation', 'multi-location-product-and-inventory-management-pro') . '</a>';
            $our_plugins_link = '<a href="' . esc_url(admin_url('admin.php?page=plugincy-plugins')) . '">' . esc_html__('Our Plugins', 'multi-location-product-and-inventory-management-pro') . '</a>';
        ...
        public static function plugin_row_meta($links, $file)
        {
            ...
            $row_meta = array(
                'docs'    => '<a href="' . esc_url($docs_url) . '" aria-label="' . esc_attr__('View documentation', 'multi-location-product-and-inventory-management-pro') . '">' . esc_html__('Docs', 'multi-location-product-and-inventory-management-pro') . '</a>',
                'support' => '<a href="' . esc_url($support_url) . '" aria-label="' . esc_attr__('Support', 'multi-location-product-and-inventory-management-pro') . '">' . esc_html__('Support', 'multi-location-product-and-inventory-management-pro') . '</a>',
                'community_support' => '<a href="' . esc_url($community_support_url) . '" aria-label="' . esc_attr__('Visit community forums', 'multi-location-product-and-inventory-management-pro') . '">' . esc_html__('Community support', 'multi-location-product-and-inventory-management-pro') . '</a>',
            );
        ...
        public function enqueue_scripts()
        {
            ...
            wp_enqueue_style('mulopimfwc_style', plugins_url('assets/css/style.css', __FILE__), [], '1.1.8');
            ...
            wp_localize_script('mulopimfwc_script', 'mulopimfwc_locationWiseProducts', [
                ...
                'location_change_notification' => isset($mulopimfwc_options["location_change_notification"]) && $mulopimfwc_options["location_change_notification"] === "on",
        ...
        private function normalize_location_slugs(array $locations): array
        {
            $locations = array_map(function ($location_slug) {
                return sanitize_title(rawurldecode((string) $location_slug));
            }, $locations);

            $locations = array_values(array_filter($locations, function ($location_slug) {
                return is_string($location_slug) && $location_slug !== '';
            }));
        ...
        private function current_user_has_unrestricted_admin_product_access(): bool
        {
        ...
        private function get_backend_location_manager_product_scope()
        {
            if (!is_user_logged_in() || !class_exists('MULOPIMFWC_Location_Managers')) {
                return null;
            }

            if ($this->current_user_has_unrestricted_admin_product_access()) {
                return null;
            }
            ...
            return $this->normalize_location_slugs((array) MULOPIMFWC_Location_Managers::get_user_assigned_locations());
        }

        private function is_admin_origin_rest_request($request = null): bool
        {
            ...
            if (
                ...
                (string) $request->get_param('context') === 'edit'
            ) {
                return true;
            }
        ...
        private function is_backend_product_request_context($request = null): bool
        {
            if (is_admin()) {
                return true;
            }

            return $this->is_admin_origin_rest_request($request);
        }

        private function product_matches_location_scope($product_id, array $location_scope): bool
        {
            $location_scope = $this->normalize_location_slugs($location_scope);
            if (empty($location_scope)) {
                return false;
            }

            $terms = $this->get_product_location_slugs((int) $product_id);
            if (empty($terms)) {
                return false;
            }

            return !empty(array_intersect($location_scope, $terms));
        }

        private function filter_products_to_location_scope(array $products, array $location_scope): array
        {
            $location_scope = $this->normalize_location_slugs($location_scope);
            if (empty($location_scope)) {
                return [];
            ...
            foreach ($products as $id => $product) {
                if (!$this->product_matches_location_scope((int) $id, $location_scope)) {
                    unset($products[$id]);
                }
            }
        ...
        public function add_location_filtering_and_ordering_to_all_queries($clauses, $query)
        {
            ...
            if ($this->is_admin_origin_rest_request()) {
                return $clauses;
            }
        ...
        public function location_selector_shortcode($atts)
        {
            ...
            global $mulopimfwc_locations;
            $atts = shortcode_atts([
                'title' => __('Select Store Location', 'multi-location-product-and-inventory-management-pro'),
                'show_title' => 'on',
                'class' => '',
                'show_button' => 'off',
                'use_select2' => 'off',
        ...
        public function display_popup_shortcode($atts)
        {
            ...
            $atts = shortcode_atts([
                ...
                'button_title' => __('Open Modal', 'multi-location-product-and-inventory-management-pro')
            ], $atts);
        ...
        {
            // Backend product searches must not inherit the frontend selected-location cookie.
            // Only location managers keep admin-side scoping, and that scope is based on their
            // assigned locations instead of the selected location cookie.
            if ($this->is_backend_product_request_context()) {
                $backend_scope = $this->get_backend_location_manager_product_scope();
                if (!is_array($backend_scope)) {
                    return $products;
                }

                return $this->filter_products_to_location_scope((array) $products, $backend_scope);
            }
        ...
        public function filter_rest_api_products($args, $request)
        {
            ...
            if ($this->is_backend_product_request_context($request)) {
                $backend_scope = $this->get_backend_location_manager_product_scope();
                if (!is_array($backend_scope)) {
                    return $args;
                }

                if (empty($backend_scope)) {
                    $args['post__in'] = [0];
                    return $args;
                }
                ...
                $args['tax_query'][] = $this->build_location_tax_query($backend_scope, 'off', true);
                return $args;
            }
        ...
        public function modify_product_rest_response($response, $product, $request)
        {
            if ($this->is_backend_product_request_context($request)) {
                ...
                if ($this->product_matches_location_scope($product->get_id(), $backend_scope)) {
                    return $response;
                }
        ...
        {
            // Validate request
            check_ajax_referer('multi-location-product-and-inventory-management-pro', 'nonce');
        ...
        {
            wp_enqueue_style('mulopimfwc-custom-admin-style', plugin_dir_url(__FILE__) . 'assets/css/admin-style.css', array(), "1.1.8");
        }
                ...
                $(document).on('change', '.mulopimfwc-order-item-location-select', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                if (priceChanged) {
                                    ...
                                    \$updating.text('" . esc_js(__('Price updated! Reloading...', 'multi-location-product-and-inventory-management-pro')) . "').css('color', '#00a32a');
                                    setTimeout(function() {
                                        window.location.reload();
                                    }, 1000);
                                } else {
                                    // Only location changed - show success message
                                    \$updating.text('" . esc_js(__('Updated!', 'multi-location-product-and-inventory-management-pro')) . "').css('color', '#00a32a');
                                    
                                    // Hide message after 2 seconds
                                    setTimeout(function() {
                                        \$updating.hide().text('" . esc_js(__('Updating...', 'multi-location-product-and-inventory-management-pro')) . "').css('color', '#2271b1');
                                    }, 2000);
                                ...
                                \$select.val(originalValue);
                                var errorMessage = response.data && response.data.message ? response.data.message : '" . esc_js(__('Failed to update location', 'multi-location-product-and-inventory-management-pro')) . "';
                        ...
                        error: function() {
                            // Revert selection on error
                            \$select.val(originalValue);
                            alert('" . esc_js(__('An error occurred while updating the location', 'multi-location-product-and-inventory-management-pro')) . "');
                        },
                        complete: function() {
                            // Re-enable select
    ...
    if (!function_exists('mulopimfwc_get_dashboard_cache_bump_interval')) {
        function mulopimfwc_get_dashboard_cache_bump_interval(): int
        {
            // Dashboard data must invalidate immediately after writes. We keep the
            // filter for backward compatibility, but the default behavior is now
            // "bump once per request, never defer across requests".
            $interval = (int) apply_filters('mulopimfwc_dashboard_cache_bump_interval', 0);
    ...
    if (!function_exists('mulopimfwc_clear_dashboard_cache')) {
        ...
        function mulopimfwc_clear_dashboard_cache(...$args): void
        {
            ...
            delete_option('mulopimfwc_dashboard_cache_last_bump');
    ...
    if (!function_exists('mulopimfwc_clear_dashboard_cache_on_product_object_save')) {
        function mulopimfwc_clear_dashboard_cache_on_product_object_save($product): void
        {
            ...
            mulopimfwc_clear_dashboard_cache('product_object_save', $product_id);
        }
    }

    if (!function_exists('mulopimfwc_clear_dashboard_cache_on_order_object_save')) {
        function mulopimfwc_clear_dashboard_cache_on_order_object_save($order): void
        {
            ...
            mulopimfwc_clear_dashboard_cache('order_object_save', $order_id);
        }
    }

    if (!function_exists('mulopimfwc_clear_dashboard_cache_on_order_item_change')) {
        function mulopimfwc_clear_dashboard_cache_on_order_item_change($item_id = 0, ...$args): void
        {
            if (!$item_id || !function_exists('wc_get_order_item')) {
                return;
            }
            ...
            mulopimfwc_clear_dashboard_cache('order_item_change', $order_id, $item_id);
        }
    }

    if (!function_exists('mulopimfwc_clear_dashboard_cache_on_location_term_change')) {
        function mulopimfwc_clear_dashboard_cache_on_location_term_change($term_id = 0, $tt_id = 0, $taxonomy = '', ...$args): void
        {
            if ($taxonomy !== 'mulopimfwc_store_location') {
                return;
            }

            mulopimfwc_clear_dashboard_cache('location_term_change', (int) $term_id, $taxonomy);
        }
    }

    if (!function_exists('mulopimfwc_clear_dashboard_cache_on_object_terms_change')) {
        function mulopimfwc_clear_dashboard_cache_on_object_terms_change($object_id, $terms, $tt_ids, $taxonomy, $append, $old_tt_ids): void
        {
            if ($taxonomy !== 'mulopimfwc_store_location') {
                return;
            }

            if (!$object_id || !mulopimfwc_is_product_like_post($object_id)) {
                return;
            }

            mulopimfwc_clear_dashboard_cache('object_terms_change', (int) $object_id, $taxonomy);
        }
    }
    ...
    add_action('woocommerce_after_product_object_save', 'mulopimfwc_clear_dashboard_cache_on_product_object_save', 10, 1);
    ...
    add_action('woocommerce_after_order_object_save', 'mulopimfwc_clear_dashboard_cache_on_order_object_save', 20, 1);
    add_action('woocommerce_saved_order_items', 'mulopimfwc_clear_dashboard_cache_on_order_event', 10, 1);
    add_action('woocommerce_new_order_item', 'mulopimfwc_clear_dashboard_cache_on_order_item_change', 10, 1);
    add_action('woocommerce_before_delete_order_item', 'mulopimfwc_clear_dashboard_cache_on_order_item_change', 10, 1);
    ...
    add_action('created_term', 'mulopimfwc_clear_dashboard_cache_on_location_term_change', 10, 3);
    add_action('edited_term', 'mulopimfwc_clear_dashboard_cache_on_location_term_change', 10, 3);
    add_action('delete_term', 'mulopimfwc_clear_dashboard_cache_on_location_term_change', 10, 3);
    add_action('set_object_terms', 'mulopimfwc_clear_dashboard_cache_on_object_terms_change', 10, 6);
    ...
    if (!function_exists('mulopimfwc_resolve_store_location')) {
        ...
        function mulopimfwc_resolve_store_location()
        {
            ...
            if (!class_exists('MULOPIMFWC_Location_Resolver')) {
                wp_send_json_error(['message' => __('Resolver service is unavailable.', 'multi-location-product-and-inventory-management-pro')]);
                return;
            }

            if (!mulopimfwc_resolver_rate_limit_allowed()) {
                wp_send_json_error([
                    'message' => __('Too many location resolve requests. Please try again shortly.', 'multi-location-product-and-inventory-management-pro'),
                    'code' => 'rate_limited',
                ]);
                return;
            }
    ...
    function mulopimfwc_validate_location() {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mulopimfwc_location_validation')) {
            wp_send_json_error(array('message' => __('Invalid security token.', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        if (empty($location_slug)) {
            wp_send_json_error(array('message' => __('Location slug is required.', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        $term = function_exists('mulopimfwc_validate_location_slug')
            ...
            : get_term_by('slug', $location_slug, 'mulopimfwc_store_location');
        if (!$term || is_wp_error($term)) {
            wp_send_json_error(array('message' => __('Invalid location.', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        
        wp_send_json_success(array('message' => __('Location validated.', 'multi-location-product-and-inventory-management-pro')));
    }
    ...
    if (!function_exists('mulopimfwc_update_wc_customer_address')) {
        /**
         * Resolve which WooCommerce customer address should be updated.
         *
         * @return string shipping|billing
         */
        function mulopimfwc_get_wc_customer_address_target()
        {
            $use_shipping = false;
        ...
        /**
         * Read the active WooCommerce customer address from the session.
         *
         * @param string $target shipping|billing
         * @return array
         */
        function mulopimfwc_get_wc_customer_address_snapshot($target = '')
        {
            if (!function_exists('WC') || !WC() || !WC()->customer) {
                return [];
            }

            $target = $target === 'billing' ? 'billing' : ($target === 'shipping' ? 'shipping' : mulopimfwc_get_wc_customer_address_target());
            $customer = WC()->customer;
        ...
        /**
         * Normalize the comparable parts of a customer address.
         *
         * @param array $address
         * @return array
         */
        function mulopimfwc_prepare_customer_address_for_comparison(array $address)
        {
        ...
        /**
         * Check whether an address currently contains any meaningful data.
         *
         * @param array $address
         * @return bool
         */
        function mulopimfwc_customer_address_has_data(array $address)
        {
        ...
        function mulopimfwc_update_wc_customer_address(array $address)
        {
            ...
            $address = mulopimfwc_normalize_customer_address_for_woocommerce($address);
            $target = mulopimfwc_get_wc_customer_address_target();
    ...
    if (!function_exists('mulopimfwc_maybe_sync_customer_address_from_selected_location')) {
        ...
        function mulopimfwc_maybe_sync_customer_address_from_selected_location()
        {
            ...
            if (WC()->session && is_object(WC()->session)) {
                ...
                $seeded_address = WC()->session->get('mulopimfwc_location_seeded_customer_address', []);
                if (is_array($seeded_address)) {
                    $last_seeded_target = sanitize_key((string) ($seeded_address['target'] ?? ''));
                    $last_seeded_address = isset($seeded_address['address']) && is_array($seeded_address['address'])
                        ? mulopimfwc_prepare_customer_address_for_comparison($seeded_address['address'])
                        : [];
                }
            }

            $current_target = mulopimfwc_get_wc_customer_address_target();
            $current_address = mulopimfwc_prepare_customer_address_for_comparison(
                mulopimfwc_get_wc_customer_address_snapshot($current_target)
            );
            $current_address_has_data = mulopimfwc_customer_address_has_data($current_address);

            // Only keep syncing while the checkout address still matches the last location-seeded value.
            if ($current_address_has_data) {
                if ($last_seeded_target !== $current_target || empty($last_seeded_address) || $current_address !== $last_seeded_address) {
                    return;
                }
            }

            if ($last_synced_location === $location_slug && $current_address_has_data) {
                return;
            }
            ...
            if (WC()->session && is_object(WC()->session)) {
                ...
                WC()->session->set('mulopimfwc_location_seeded_customer_address', [
                    'target' => $updated,
                    'address' => mulopimfwc_prepare_customer_address_for_comparison($address),
                ]);
            }
        }
    }
    ...
    function mulopimfwc_update_customer_address()
    {
        ...
        $options = get_option('mulopimfwc_display_options', []);
        if (!isset($options['auto_populate_customer_addresses']) || $options['auto_populate_customer_addresses'] !== 'on') {
            wp_send_json_error(['message' => __('Auto-populate addresses is disabled.', 'multi-location-product-and-inventory-management-pro')]);
            return;
        }

        if (!function_exists('WC')) {
            wp_send_json_error(['message' => __('WooCommerce is not available.', 'multi-location-product-and-inventory-management-pro')]);
            return;
        }
        ...
        if (!WC()->customer) {
            wp_send_json_error(['message' => __('Customer session unavailable.', 'multi-location-product-and-inventory-management-pro')]);
            return;
        }
        ...
        if (!$has_data) {
            wp_send_json_error(['message' => __('No address data provided.', 'multi-location-product-and-inventory-management-pro')]);
            return;
        }
        ...
        if (WC()->session && is_object(WC()->session)) {
            WC()->session->set('mulopimfwc_autopopulated_location', '');
            WC()->session->set('mulopimfwc_location_seeded_customer_address', []);
        }
    ...
    function mulopimfwc_delete_user_location()
    {
        ...
        if (!is_user_logged_in()) {
            wp_send_json_error(array('message' => __('You must be logged in to delete locations.', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        if (empty($location_id)) {
            wp_send_json_error(array('message' => __('Invalid location ID', 'multi-location-product-and-inventory-management-pro')));
            return;
        }

        // FIXED: Validate location ID format (Issue #37)
        if (!preg_match('/^[a-zA-Z0-9_-]+$/', $location_id)) {
            wp_send_json_error(array('message' => __('Invalid location ID format', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        if (!is_array($user_locations)) {
            wp_send_json_error(array('message' => __('No saved locations found', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        if (!$found) {
            wp_send_json_error(array('message' => __('Location not found', 'multi-location-product-and-inventory-management-pro')));
            return;
        }
        ...
        wp_send_json_success(array('message' => __('Location deleted successfully', 'multi-location-product-and-inventory-management-pro')));
    }
            ...
            [
                ...
                'status_label' => $status === 'out_of_stock'
                    ...
                    : ($status === 'low_stock'
                        ...
                        : __('In Stock', 'multi-location-product-and-inventory-management-pro')),
            ]
        );
    }
    ...
    function mulopimfwc_build_stock_display_label($args = [])
    {
        ...
        $backorder_label = ($args['backorder_label'] === 'available')
            ...
            : __('Backorder', 'multi-location-product-and-inventory-management-pro');

        if ($format === 'availability_only' || $stock_qty === null) {
            if ($status === 'instock') {
                $label = __('In stock', 'multi-location-product-and-inventory-management-pro');
            ...
            if ($status === 'onbackorder') {
                ...
                if ($stock_qty <= $out_threshold) {
                    ...
                    $label = __('Low stock', 'multi-location-product-and-inventory-management-pro');
                    $level = 'low';
                    $class = 'low';
                } else {
                    ...
                    if ($stock_qty <= $medium_threshold) {
                        $label = __('Medium stock', 'multi-location-product-and-inventory-management-pro');
                        $level = 'medium';
                        $class = 'medium';
                    } else {
                        $label = __('High stock', 'multi-location-product-and-inventory-management-pro');
                        $level = 'high';
                        $class = 'high';
                    }
                }
    ...
    function mulopimfwc_send_location_stock_alert($product_id, $location_term, $current_stock, $threshold, $type = 'low')
    {
        ...
        $type_label = ($type === 'out') ? __('Out of Stock', 'multi-location-product-and-inventory-management-pro') : __('Low Stock', 'multi-location-product-and-inventory-management-pro');
    ...
    function mulopimfwc_handle_social_new_order($order_id)
    {
        ...
        $location_name = $location_term && !is_wp_error($location_term) ? $location_term->name : __('Unassigned location', 'multi-location-product-and-inventory-management-pro');
    ...
    function mulopimfwc_get_daily_social_digest_stats()
    {
        ...
        foreach ($all_orders as $order) {
            ...
            if (!isset($stats[$slug])) {
                ...
                $stats[$slug] = [
                    'name' => ($term && !is_wp_error($term)) ? $term->name : __('Unassigned location', 'multi-location-product-and-inventory-management-pro'),
                    'orders' => 0,
                    'items' => 0,
                    'revenue' => 0.0,
                ];
        ...
        if (!empty($admin_channels)) {
            ...
            foreach ($stats as $data) {
                $lines[] = sprintf(
                    ...
                    $data['name'],
                    $data['orders'],
                    __('orders', 'multi-location-product-and-inventory-management-pro'),
                    $data['items'],
                    __('items', 'multi-location-product-and-inventory-management-pro'),
                    wp_strip_all_tags(wc_price($data['revenue']))
                );
            }

            mulopimfwc_send_social_message(
                __("Today's location performance", 'multi-location-product-and-inventory-management-pro'),
                implode("\n", $lines),
                $admin_channels,
                $settings,
    ...
    {
        mulopimfwc_social_order_status_alert($order_id, 'payment_failed', __('Payment failed', 'multi-location-product-and-inventory-management-pro'));
    }
    function mulopimfwc_social_order_refunded($order_id)
    {
        mulopimfwc_social_order_status_alert($order_id, 'order_refunded', __('Order refunded', 'multi-location-product-and-inventory-management-pro'));
    }
    function mulopimfwc_social_order_cancelled($order_id)
    {
        mulopimfwc_social_order_status_alert($order_id, 'order_cancelled', __('Order cancelled', 'multi-location-product-and-inventory-management-pro'));
    }
    function mulopimfwc_social_order_completed($order_id)
    {
        mulopimfwc_social_order_status_alert($order_id, 'order_completed', __('Order completed', 'multi-location-product-and-inventory-management-pro'));
    }
    ...
    add_filter('cron_schedules', function ($schedules) {
        $schedules['mulopimfwc_five_minutes'] = [
            'interval' => 300,
            'display' => __('Every 5 Minutes', 'multi-location-product-and-inventory-management-pro')
        ];
        return $schedules;
    });
    ...
    function mulopimfwc_test_social_channel()
    {
        ...
        if (empty($type)) {
            wp_send_json_error(['message' => __('Select a platform to test.', 'multi-location-product-and-inventory-management-pro')]);
        }

        if ($type === 'telegram' && empty($chat_id) && empty($webhook)) {
            wp_send_json_error(['message' => __('Telegram requires a chat ID (and bot token).', 'multi-location-product-and-inventory-management-pro')]);
        }

        if ($type !== 'telegram' && empty($webhook)) {
            wp_send_json_error(['message' => __('Webhook URL is required for this platform.', 'multi-location-product-and-inventory-management-pro')]);
        }
        ...
        mulopimfwc_send_social_message(
            __('Test notification', 'multi-location-product-and-inventory-management-pro'),
            __('This is a test from Multi Location Product & Inventory Management.', 'multi-location-product-and-inventory-management-pro'),
            [$channel],
            $settings,
            home_url()
        );

        wp_send_json_success(['message' => __('Test sent (check your channel).', 'multi-location-product-and-inventory-management-pro')]);
    }
    ...
    function mulopimfwc_test_social_digest_channel()
    {
        ...
        if (empty($stats)) {
            ...
            foreach ($stats as $data) {
                $lines[] = sprintf(
                    '%s - %d %s, %d %s, %s',
                    isset($data['name']) ? $data['name'] : __('Location', 'multi-location-product-and-inventory-management-pro'),
        ...
        wp_send_json_success(['message' => __('Digest test sent (check your channel).', 'multi-location-product-and-inventory-management-pro')]);
    }
        ...
        if (!empty($channels)) {
            mulopimfwc_send_social_message(
                ...
                sprintf(
                    ...
                    __('%1$s received a %2$s-star review: "%3$s"', 'multi-location-product-and-inventory-management-pro'),
                    $product_name,
                    $rating,
...
add_action('wp_enqueue_scripts', 'mulopimfwc_enqueue_admin_notifications_assets');
...
function mulopimfwc_enqueue_admin_notifications_assets()
{
    ...
    $admin_notifications_js = plugin_dir_path(__FILE__) . 'assets/js/admin-notifications.js';
...
function mulopimfwc_send_test_notification_email_ajax()
{
    ...
    if (($type === 'location_contact' || $type === 'location_manager') && empty($location_slug)) {
        wp_send_json_error(['message' => __('Please select a location.', 'multi-location-product-and-inventory-management-pro')]);
        return;
    }
    ...
    if ($type === 'admin_email') {
        ...
        if ($custom_email && is_email($custom_email)) {
            ...
            wp_send_json_error(['message' => __('Please enter a valid email address.', 'multi-location-product-and-inventory-management-pro')]);
            return;
        }
    ...
    if (empty($recipients)) {
        wp_send_json_error(['message' => __('No valid recipient email could be resolved for this test.', 'multi-location-product-and-inventory-management-pro')]);
        return;
    }

    $site_name = wp_specialchars_decode(get_bloginfo('name'), ENT_QUOTES);
    $subject = sprintf('[%s] %s', $site_name, __('Test notification email', 'multi-location-product-and-inventory-management-pro'));
    ...
    if (!$sent_any) {
        wp_send_json_error(['message' => __('wp_mail() reported failure. Please check your SMTP/mail configuration.', 'multi-location-product-and-inventory-management-pro')]);
        return;
    }
...
function mulopimfwc_check_for_plugin_updates($transient, $license_manager)
{
    ...
    $current_version = defined('MULOPIMFWC_VERSION') ? MULOPIMFWC_VERSION : '1.1.8';
